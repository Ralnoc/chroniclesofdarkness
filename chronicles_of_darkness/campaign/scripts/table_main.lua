...
function onOptionUpdate()
	if User.isHost() then
		...
		hiderollresults.setVisible(false);
	end
end
...
function updateDieHeader()
	...
	header2.subwindow.header_die.setValue(sDice);
end
...
function addColumnLabel(index)
	local ctrlLabel = header2.subwindow["labelcol" .. index];
	if not ctrlLabel then
		ctrlLabel = header2.subwindow.createControl("label_tablecolumn", "labelcol" .. index);
		if ctrlLabel then
...
function onDrop(x, y, draginfo)
	...
	-- Check to see if roll should be hidden
	rTableRoll.bSecret = (DB.getValue(rTableRoll.nodeTable, "hiderollresults", 0) == 1);
...
function actionRoll(draginfo)
	local rTableRoll = {};
	rTableRoll.nodeTable = getDatabaseNode();
	rTableRoll.bSecret = (hiderollresults.getValue() == 1);
