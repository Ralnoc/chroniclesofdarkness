...
function addItem(sClass, nodeRecord, bTransferAll)
	ItemManager.notifyTransfer("partysheet.treasureparcelitemlist", sClass, nodeRecord.getNodeName(), bTransferAll)
end
...
function sellItems()
	...
	for _,vItem in pairs(DB.getChildren("partysheet.treasureparcelitemlist")) do
		...
		if sAssign == "" then
			...
			local sCoinValue, sCoin = string.match(sCost, "^%s*([%d,]+)%s*([%w%s]*)$");
			if sCoinValue then
