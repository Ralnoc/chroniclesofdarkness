...
function handlePCDrop(nodeChar, draginfo, sTarget)
	if CampaignDataManager2 and CampaignDataManager2.handlePCDrop then
		if CampaignDataManager2.handlePCDrop(nodeChar, draginfo) then
			return true;
		end
	end
	
	if not User.isHost() and not DB.isOwner(nodeChar) then
		return;
	end
	...
	if ItemManager.isItemClass(sClass) then
		if not sTarget or sTarget == "items" then
			ItemManager.notifyTransfer(DB.getPath(nodeChar, "inventorylist"), sClass, sRecord, Input.isShiftPressed());
			return true;
		end
	elseif sClass == "treasureparcel" then
		if not sTarget or sTarget == "items" then
			local nodeParcel = draginfo.getDatabaseNode();
			
			local sCharInvList = DB.getPath(nodeChar, "inventorylist");
			for _,vParcelItem in pairs(DB.getChildren(nodeParcel, "itemlist")) do
				ItemManager.notifyTransfer(sCharInvList, "item", vParcelItem.getNodeName(), true);
			end
			...
			for _,vParcelCoin in pairs(DB.getChildren(nodeParcel, "coinlist")) do
				...
				PartyLootManager.addCoinsToPC(nodeChar, sCoin, nCoin);
			end

			local msg = {font = "msgfont", icon = "coins"};
			msg.text = "Parcel [" .. DB.getValue(DB.getPath(sRecord, "name"), "") .. "] -> [" .. DB.getValue(DB.getPath(nodeChar, "name"), "") .. "]";
