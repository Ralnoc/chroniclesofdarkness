...
function updateControls()
	...
	local neededheight = (dockspacing + dockiconsize[2]) * controlcount + docklowerspacing;
	
	local scaling = 1;
	if availableheight < neededheight then
		scaling = (availableheight - docklowerspacing - dockspacing * controlcount) / (dockiconsize[2] * controlcount);
	end
	...
	for k,v in pairs(subdockcontrols) do
		v.setStaticBounds(dockoffset[1] + (1-scaling)*dockiconsize[1]/2, winh - dockspacing*(k-1) - math.floor(dockiconsize[2]*scaling) * k + dockoffset[2] - docklowerspacing, math.floor(dockiconsize[1]*scaling), math.floor(dockiconsize[2]*scaling));
	end
end
...
function registerStackShortcut2(iconNormal, iconPressed, sTooltipRes, className, recordName)
	function createFunc()
		createStackShortcut(iconNormal, iconPressed, Interface.getString(sTooltipRes), className, recordName);
	end
...
function registerDockShortcut2(iconNormal, iconPressed, sTooltipRes, className, recordName, useSubdock)
	function createFunc()
		createDockShortcut(iconNormal, iconPressed, Interface.getString(sTooltipRes), className, recordName, useSubdock);
	end
